...
interface DruidClass {
    ...
    new_blocker: (node: string | node) => DruidBlocker;
    ...
    new_swipe(node: node | string, on_swipe_callback: (self: any, direction: string, distance: any, swipe_time: any) => void): DruidNode;
}
...
interface DruidGridVertical extends DruidGrid {
    add: (node: node, index: number, is_inst?: boolean) => void;
...
interface DruidRadioGroup extends DruidNode {
    set_state: (index: number, is_instant: boolean) => void;
    get_state: () => number;
}

interface DruidCheckBox extends DruidNode {
    get_state: () => boolean;
