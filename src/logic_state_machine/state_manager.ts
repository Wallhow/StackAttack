...
export function StateManager(transition_info_cb: TransitionInfoCallback) {
    ...
    function update_states(id: number, mask: number, index: number, force_update = false) {
        ...
        if (old_state == null || (old_state.index != new_state.index || old_state.mask != new_state.mask) || force_update)
    ...
    return { set_go_list, update_states, apply_state, configure_seq_item, configure_seq_list_items, transition_manager };
}