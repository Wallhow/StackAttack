...
export function GoManager<GameItemType extends IGameItem = IGameItem>() {

    let go_list: hash[] = [];
    let game_items: GameItemType[] = [];

    let index = 0;
    let index2GameItem: { [key in number]: GameItemType } = {};
    ...
    function get_go_by_item(item: GameItemType) {
    ...
    function set_render_order(item: GameItemType, index: number) {
    ...
    function get_render_order(item: GameItemType) {
    ...
    function do_move_anim(item: GameItemType, pos: vmath.vector3, timeSec: number, delay = 0) {
    ...
    function do_scale_anim(item: GameItemType, scale: vmath.vector3, timeSec: number, delay = 0) {
    ...
    function do_fade_anim_hash(_go: hash, value: number, timeSec: number, delay = 0, prop = 'tint') {
        go.animate(msg.url(undefined, _go, 'sprite'), prop + '.w', go.PLAYBACK_ONCE_FORWARD, value, go.EASING_LINEAR, timeSec, delay);
    }

    function do_fade_anim(item: GameItemType, value: number, timeSec: number, delay = 0, prop = 'tint') {
    ...
    function set_position_xy(item: GameItemType, x: number, y: number, align_x = 0.5, align_y = 0.5) {
    ...
    function move_to_with_speed(item: GameItemType, pos: vmath.vector3, speed: number, cb?: CallbackFunction) {
    ...
    function is_intersect(pos: vmath.vector3, item: GameItemType, inner_offset?: vmath.vector3) {
        return is_intersect_hash(pos, get_go_by_item(item), inner_offset);
    }

    function get_item_from_pos(x: number, y: number): null | [GameItemType, GameItemType[]] {
    ...
    let down_item: GameItemType | null = null;
    ...
    function add_game_item<T extends GameItemType>(gi: T, add_go_list = true): number {
        game_items.push(gi);
        if (add_go_list)
    ...
    function delete_item(item: GameItemType, remove_from_scene = true, recursive = false) {
